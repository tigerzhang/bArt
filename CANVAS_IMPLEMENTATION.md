    ...
    - Panel fold/unfold pattern
      ...
      - Implementation details:
        ...
        - Toggle placement:
          ...
          - Visual tuning: Use CSS variables to nudge the fold control to precisely match the toggle. For example, `--panel-fold-left-adjust`, `--panel-fold-bottom-adjust`, `--panel-fold-top-adjust`, and `--panel-fold-right-adjust` are available so you can fine-tune offsets without changing toolbar layout. Default is 0px.
        - On open: render the panel; apply `.enter` CSS class so it animates into view. On fold/close: set an `exiting` flag, add `.exit` CSS class and wait for the `transitionend` event (instead of a hard-coded timeout) to toggle the store state so the unmount is robust to animation changes.
